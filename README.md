...
| `CLAUDE.md` | A two-line `@AGENTS.md` import, plus anything genuinely Claude-specific. |
...
| `.claude/skills/` | Claude Code skills: `/gates`, `/new-crate`, `/why`. |
...
from, so a rule is always traceable to one file upstream. To find which,
run `agentcfg why "<phrase>"`. To change a rule for every repository, open a pull
request against that fragment. To change it for this one only, put it outside the
`<!-- agentcfg:start -->` / `<!-- agentcfg:end -->` markers — regeneration never
touches what sits outside them.
